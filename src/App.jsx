...
function App() {
  ...
  const fetchData = () => {
    dispatch(
      ...
      .then((res) => {
        // Safely check if the payload exists and if there's no error
        if (
          res.payload &&
          !res.payload.error &&
          res.payload.data &&
          res.payload.data.coord
        ) {
          // Proceed to dispatch get5DaysForecast if all conditions are met
          dispatch(
            get5DaysForecast({
          ...
          // Handle the error or missing data case
          console.error(
            "Error fetching city data or coordinates not available:",
            res.payload ? res.payload.error : "Unknown error"
          );
        }
      })
      .catch((error) => {
        // Handle any dispatch-related errors or promise rejections
